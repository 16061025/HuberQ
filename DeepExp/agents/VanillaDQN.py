...
class VanillaDQN(BaseAgent):
    ...
    def __init__(self, cfg):
        ...
        self.action_multip = int(cfg['action_multiple'])
        self.original_action_size = self.get_action_size()
        self.action_size = self.original_action_size * self.action_multip # create more action
        ...
        # Set log dict
        for key in ['state', 'next_state', 'action', 'reward', 'original_reward', 'done', 'truncated', 'terminated', 'episode_return', 'episode_step_count']:
            setattr(self, key, {'Train': None, 'Test': None})

        self.hubersigma=100
        self.huberepsilon=0.01
    ...
    def reset_game(self, mode):
        ...
        self.original_state,_ = self.env[mode].reset() # state before processed
    ...
    def run_episode(self, mode, render):
        while not self.done[mode]:
            ...
            self.action[mode] = self.action[mode] % self.original_action_size
            if render:
                self.env[mode].render()
            # Take a step
            next_state, self.reward[mode], self.terminated[mode], self.truncated[mode], _ = self.env[mode].step(self.action[mode])
            self.done[mode] = self.terminated[mode] or self.truncated[mode]
            self.next_state[mode] = self.state_normalizer(next_state)
            self.reward[mode] = self.reward_normalizer(self.reward[mode])
            self.original_reward[mode] = self.reward[mode]
            if np.random.rand() < self.huberepsilon:
                self.reward[mode] = self.reward[mode] + np.random.normal(0, self.hubersigma)
                print(self.reward[mode])
                print(self.original_reward[mode])
            self.episode_return[mode] += self.original_reward[mode]
